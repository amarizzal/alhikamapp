...
this.c=$.extend(!0,{},Buttons.defaults,config);if(config.buttons){this.c.buttons=config.buttons}
...
jqNode.toggleClass(klass,flag===undefined?!0:flag);return this},add:function(config,idx)
...
this._expandButton(buttons,config,!1,idx);this._draw();return this},container:function()
...
{if(flag===!1){return this.disable(node)}
...
if(built.conf.buttons){var collectionDom=this.c.dom.collection;built.collection=$('<'+collectionDom.tag+'/>').addClass(collectionDom.className).attr('role','menu');built.conf._collection=built.collection;this._expandButton(built.buttons,built.conf.buttons,!0,attachPoint)}
...
if(config.available&&!config.available(dt,config)){return!1}
...
if(config.enabled===!1){button.addClass(buttonDom.disabled)}
...
if(typeof base==='function'){base=base(dt,conf);if(!base){return!1}}
...
{var config=$.extend(!0,{},{rows:null,columns:'',modifier:{search:'applied',order:'applied'},orthogonal:'display',stripHtml:!0,stripNewlines:!0,decodeEntities:!0,trim:!0,format:{header:function(d){return strip(d)},footer:function(d){return strip(d)},body:function(d){return strip(d)}}},inOpts);var strip=function(str){if(typeof str!=='string'){return str}
